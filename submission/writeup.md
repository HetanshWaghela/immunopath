...
> | PD-L1 IHC | $300–800 | 3–7 days | ⚠️ Limited |
...
>                     ▼               ▼               ▼
>          ┌──────────────┐ ┌────────────────┐ ┌──────────────┐
>          │  🧠 MedGemma  │ │ 🔬 Path Found. │ │ 🎯 MedSigLIP │
>          │  (fine-tuned) │ │  (embeddings)  │ │ (zero-shot)  │
>          │   4B · LoRA   │ │ 384-dim · TF   │ │  CLIP-style  │
>          └──────┬───────┘ └───────┬────────┘ └──────┬───────┘
>                 │                 │                  │
>                 ▼                 ▼                  ▼
>     ┌───────────────────┐  Visual Embed.    Phenotype Scores
>     │ 8 Immune Biomarkers│  for similarity   (inflamed/excluded
>     │  (structured JSON) │  analysis          /desert)
>     └────────┬──────────┘
>              ▼
>     ┌───────────────────┐
>     │ 📋 Guideline Engine │──────────────────┐
...
> | Trainable params | 0.78% | 46.56% ⚠️ | **2.37%** |
...
Training: A100-80GB, 77 minutes, 3 epochs with early stopping (patience=3). TIL MAE improved from 0.26 to **0.16** — meeting the clinical target of <0.20. TIL Spearman shifted from -0.17 to **+0.12**, confirming directional correlation with ground truth. Response-only masking was the single largest contributor to this improvement.
...
> │                                     │  │                                     │
> │  MedGemma 4B (4-bit)    ~3.2 GB    │  │  TxGemma 9B (4-bit)     ~6.0 GB   │
> │  MedSigLIP              ~0.4 GB    │  │                                     │
> │  Path Foundation (TF)   ~1.2 GB    │  │  [Reserved for inference] ~9.0 GB  │
> │                                     │  │                                     │
> │  ⚠️ TF restricted to GPU 0 via     │  │                                     │
...
Full pipeline: **~12 seconds per patient** on L4 GPU — MedGemma (~8s), Path Foundation (~1s), MedSigLIP (~1s), TxGemma (~2s).
...
> ```
> Patient diagnosed ──▶ H&E slide prepared ──▶ ImmunoPath analyzes ──▶ "CD274-high, inflamed
>   with NSCLC             (routine, $5)          patches (12s)         → expedite PD-L1 IHC"
>                                                                              │
>                                                                              ▼
...
|-------|-------------|------------|
| 0 | MedGemma capability testing | Multi-image ✅, JSON 100% |
...
| 6 | Evaluation (v2, v3.1) | TIL MAE 0.16 ✅, 100% compliance |
